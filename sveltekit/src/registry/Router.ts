  ...
  private matchSegments(
    ...
    while (routeIdx < parsed.segments.length) {
      ...
      if (type === SegmentType.Static) {
        if (pathSegment !== routeSegment) return null;
        pathIdx++;
    ...
    if (
      ...
      parsed.types[parsed.types.length - 1] !== SegmentType.Wildcard
    ) {
      return null;
    }
  ...
  private resolveLayouts(path: string): IRoute[] {
    ...
    for (const parsed of this.routes) {
      if (parsed.original.type === "layout") {
        ...
        if (
          path.startsWith(parsed.original.path) ||
          parsed.original.path === "/"
        ) {
          // Ensure implementation details correct (boundary check)
          const isRoot = parsed.original.path === "/";
          const isBoundary =
            path[parsed.original.path.length] === "/" ||
            path.length === parsed.original.path.length;
